...
test.describe.serial('THE FLOOR release path', () => {
  ...
  test('mobile navigation, briefing dialog, and focus return work', async ({ page }) => {
    ...
    const trigger = page.getByRole('button', { name: /open the role briefing/i }).first()
    await trigger.click()
    const dialog = page.getByRole('dialog', { name: /build briefing/i })
    await expect(dialog).toBeVisible()
    await expect(dialog).toContainText('Not registration')
    await expect(dialog).toContainText('No applications are open')
    await expect(dialog.getByRole('heading', { name: 'Faculty advisor' })).toBeVisible()
    await expect(dialog.getByRole('heading', { name: 'Chair / dais' })).toBeVisible()
  ...
  test('captures representative desktop and mobile evidence', async ({ page }) => {
    ...
    await page.keyboard.press('Escape')
    await page.getByRole('button', { name: /open the role briefing/i }).first().click()
